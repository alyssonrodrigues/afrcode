...
import afrcode.fwarquitetura.is.util.jms.QueueListener;
...
public class JMSBeansConfig {
     ...
     * 
     * This local strategy is an alternative to executing JMS operations within JTA transactions. Its advantage is
     * that it is able to work in any environment, for example a standalone application or a test suite, with any message
     * broker as target. However, this strategy is not able to provide XA transactions, for example in order to share
     * transactions between messaging and database access. A full JTA/XA setup is required for XA transactions, typically
     * using Spring's org.springframework.transaction.jta.JtaTransactionManager as strategy.
     * 
     * @return
     */
    ...
    public Queue queueTest() {
        Queue queueTest = new ActiveMQQueue("queue.test");
        return queueTest;
    }
