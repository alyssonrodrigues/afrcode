...
public class TratadorExcecoesNaoPrevistas implements
	...
	private void redirecionarParaPaginaDeErro(FacesContext facesContext,
			List<ExcecaoNaoPrevista> excecoes) {
	...
	private void redirecionarParaPaginaDeErroEmRequisicaoAjax(
			FacesContext facesContext, List<ExcecaoNaoPrevista> excecoes) {
	...
	private void redirecionarParaPaginaDeErroEmRequisicaoNaoAjax(
			List<ExcecaoNaoPrevista> excecoes) {
		HttpServletResponse response = getHttpServletResponse();
		if (response.isCommitted()) {
			LOG.error(MSG_ERRO_AO_REDIRECIONAR);
