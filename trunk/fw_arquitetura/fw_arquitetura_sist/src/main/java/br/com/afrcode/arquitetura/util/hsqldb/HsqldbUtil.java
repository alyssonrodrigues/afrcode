...
public class HsqldbUtil {
    ...
    private void doIniciarHSQLDB() {
        ...
        try {
            ...
            if (ServerConstants.SERVER_STATE_ONLINE == state) {
                ...
                throw new ExcecaoNaoPrevista("Não foi possível iniciar o HSQLDB por possível falha de startup "
    ...
    public Server iniciarHSQLDB() {
        if (isExecutandoPovoadorStandAlone() || isExecutandoTesteFuncional() || isExecutandoTesteIntegracao()
    ...
    private int iniciarHSQLDBParaTUsEmNovaPortaSeNecessario(int state) {
        if (isProfileTUAtivo() && ServerConstants.SERVER_STATE_SHUTDOWN == state) {
            ...
            final int cem = 100;
            final int um = 1;
            int port = ServerConstants.SC_DEFAULT_HSQL_SERVER_PORT + random.nextInt(cem) + um;
    ...
    public boolean isEjbSpringApplicationContext() {
        return ApplicationContextUtils.isEjbSpringApplicationContext(applicationContext);
    }
    ...
    public boolean isHsqldbEmUsoParaAplicacao() {
        ...
        boolean hsqldbEmUsoParaAplicacao =
                StringUtils.isNotBlank(hibernateDialect) ? hibernateDialect.equals(HSQLDB_DIALECT) : false;
    ...
    public boolean isHsqldbEmUsoParaTU() {
        ...
        boolean hsqldbEmUsoParaTU =
                StringUtils.isNotBlank(hibernateDialectTU) ? hibernateDialectTU.equals(HSQLDB_DIALECT) : false;
        return isProfileTUAtivo() && hsqldbEmUsoParaTU;
    }

    private boolean isProfileAplicacaoAtivo() {
        return Profiles.isProfileAplicacaoAtivo(applicationContext.getEnvironment());
    }
