 ...
 * 
 */
public class DaoUmObjetoPersistenteTest
		extends
	...
	public void testarRecuperarTodosComPaginacao() {
		...
		for (int i = 0; i < 100; i++) {
			UmObjetoPersistente umObj = daoUmObjetoPersistente
					.instanciarObjetoPersistivel();
			daoUmObjetoPersistente.salvar(umObj);
			objs.add(umObj);
		}

		Collection<UmObjetoPersistente> objsPaginaZero = daoUmObjetoPersistente
				.recuperarTodos(0, 50);
		Assert.assertTrue(
				"Deveriam ter sido recuperados 50 objetos para a página 0!",
				!objsPaginaZero.isEmpty());
		Assert.assertEquals(
				"Deveriam ter sido recuperados 50 objetos para a página 0!",
				50, objsPaginaZero.size());
		Assert.assertTrue(
				"Os objetos recuperados para a página 0 são diferentes dos objetos esperados!",
				objsPaginaZero.containsAll(objs.subList(0, 50)));

		Collection<UmObjetoPersistente> objsPaginaUm = daoUmObjetoPersistente
				.recuperarTodos(1, 50);
		Assert.assertTrue(
				"Deveriam ter sido recuperados 50 objetos para a página 1!",
				!objsPaginaUm.isEmpty());
		Assert.assertEquals(
				"Deveriam ter sido recuperados 50 objetos para a página 1!",
				50, objsPaginaUm.size());
		Assert.assertTrue(
				"Os objetos recuperados para a página 0 são diferentes dos objetos esperados!",
	...
	public void testarRecuperarObjetos() {
		...
		String qlString = "from " + UmObjetoPersistente.class.getName()
				+ " as obj where obj.descricao = :param_descricao";
		Map<String, Object> params = new HashMap<String, Object>();
		params.put("param_descricao", "O escolhido!");

		Collection<UmObjetoPersistente> objsPaginaZero = daoUmObjetoPersistente
				.recuperarObjetos(qlString, params, 0, 5);
		Assert.assertEquals(
				"Deveriam ter sido recuperado 1 objeto para a página 0!", 1,
				objsPaginaZero.size());
		UmObjetoPersistente[] vetorObjsPaginaZero = objsPaginaZero
				.toArray(new UmObjetoPersistente[objsPaginaZero.size()]);
		Assert.assertEquals(
				"O primeiro objeto da página 0 deveria ter a descricao igual a \"O escolhido!\"",
	...
	public void testarNotNullConstraintViolationExceptionLancadaAoSalvar() {
		...
		try {
			...
			Set<ConstraintViolation<?>> constraintViolations = cve
					.getConstraintViolations();
			Assert.assertEquals("Uma ConstraingViolation deveria existir!", 1,
					constraintViolations.size());
			ConstraintViolation<?> constraintViolation = constraintViolations
					.iterator().next();
			Assert.assertEquals(
					"Uma ConstraingViolation para NotNull deveria existir!",
					descricaoInvalida, constraintViolation.getInvalidValue());
		}
	}
	...
	public void testarNotNullConstraintViolationExistente() {
		...
		String descricaoInvalida = null;
		umObj.setDescricao(descricaoInvalida);
		Set<ConstraintViolation<UmObjetoPersistente>> constraintViolations = validator
				.validate(umObj);
		Assert.assertEquals("Uma ConstraingViolation deveria existir!", 1,
				constraintViolations.size());
		ConstraintViolation<UmObjetoPersistente> constraintViolation = constraintViolations
				.iterator().next();
		Assert.assertEquals(
				"Uma ConstraingViolation para NotNull deveria existir!",
				descricaoInvalida, constraintViolation.getInvalidValue());
	...
	public void testarPastConstraintViolationExceptionLancadaAoSalvar() {
		...
		try {
			...
			ConstraintViolation<?> constraintViolation = constraintViolations
					.iterator().next();
			Assert.assertEquals(
					"Uma ConstraingViolation para Past deveria existir!",
					dataAtual, constraintViolation.getInvalidValue());
		}
	}
	...
	public void testarCPFConstraintViolationExistente() {
		...
		ConstraintViolation<UmObjetoPersistente> constraintViolation = constraintViolations
				.iterator().next();
		Assert.assertEquals(
				"Uma ConstraingViolation para CPF deveria existir!", numCpf,
	...
	public void testarLengthConstraintValidationExistente() {
		...
		ConstraintViolation<UmObjetoPersistente> constraintViolation = constraintViolations
				.iterator().next();
		Assert.assertEquals(
				"Uma ConstraingViolation para Length deveria existir!",
				descricaoInvalida, constraintViolation.getInvalidValue());
	}
