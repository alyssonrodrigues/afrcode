...
public class SpringBatchContextTest extends AbstractCasoTesteEmMemoria {
    ...
    public void testarEntityManagerFactory() {
        Assert.assertNotNull("EntityManagerFactory indisponível!", entityManagerFactory);
    }
    ...
    public void testarLoginJAASComSucesso() {
        ...
        Assert.assertEquals("O usuário autenticado deveria ter a credencial: " + AuthorityGranterParaBatch.ROLE_USER,
                token.getCredentials(), auth.getCredentials());
        Assert.assertEquals("O usuário autenticado deveria ter a autorização:" + AuthorityGranterParaBatch.ROLE_USER,
