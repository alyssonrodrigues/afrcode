...
public class HsqldbSchemaUtil {
    ...
    private void criarDefaultSchemaSeNecessario(String nomePropriedade) {
        String nome = applicationContext.getEnvironment().getProperty(nomePropriedade);
    ...
    private void criarSchema(String nome) {
        LOG.info("Criando schema " + nome + " para uso por testes unitários ou aplicação com HSQLDB...");
        try {
    ...
    protected void criarDefaultSchemaSeTUOuAplicacaoComHsqldb() {
        if (!hsqldbUtil.isEjbSpringApplicationContext() && !hsqldbUtil.isExecutandoPovoadorStandAlone()
                && !hsqldbUtil.isExecutandoTesteFuncional() && !hsqldbUtil.isExecutandoTesteIntegracao()
                && (hsqldbUtil.isHsqldbEmUsoParaTU() || hsqldbUtil.isHsqldbEmUsoParaAplicacao())) {

            String defaultSchema =
                    hsqldbUtil.isHsqldbEmUsoParaAplicacao() ? HIBERNATE_DEFAULT_SCHEMA : HIBERNATE_DEFAULT_SCHEMA_TU;
