...
public interface IManagedBean {
     ...
     * 
     * Não é permitida a inclusão de Entidades (objetos IEntidade) em sessão HTTP - problemas relacionados: detached
     * object, stalled data, lazy initialization exceptions, resources leak, etc.
     * 
     * @param chave chave única do objeto em sessão
     * @param objetoEmSessao um objeto para adição em sessão
     * 
     * @throws IllegalArgumentException quando o objeto é na verdade uma IEntidade
     */
    void adicionarObjetoEmSessaoHttp(String chave, Object objetoEmSessao);

    /**
     * Método de remoção de objetos em sessão HTTP através de chave única identificadora.
     * 
     * @param chave
     */
    ...
    /**
     * Método de obtenção de objetos em sessão HTTP através de chave única identificadora.
     * 
     * @param chave
